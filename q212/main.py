...
class Solution:
    def find(self,board,word,i,j,process):
        if word=='':
            return True
        ...
        if board[i][j]==word[0]:
            process.append((i,j))
            res=self.find(board,word[1:],i,j+1,process) or self.find(board,word[1:],i,j-1,process) or self.find(board,word[1:],i+1,j,process) or self.find(board,word[1:],i-1,j,process)
            process.remove((i,j))
            return res
    ...
    def findWords(self,board:List[List[str]],words:List[str])->List[str]:
        results=[]
        board_set=set([item for i in board for item in i])
        i=0
        for word in words:
            if len(set(list(word))-board_set)>0:
                continue
...
if __name__ == '__main__':
    board = [["o", "a", "t", "n"], ["e", "t", "a", "e"], ["i", "h", "k", "r"], ["i", "f", "l", "v"]]
